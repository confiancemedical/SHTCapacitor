...
export class LoginPage implements OnInit {
  ...
  async verificaVersaoApp(callback: any): Promise<any> {
    ...
    if (this.plt.is("capacitor")){
      ...
      this.restProvider.validaVersao("SAT", this.appBuild)
      .then(async (data: any) => {
        const wdata: any = JSON.stringify(data);
        const jsonObj: any = JSON.parse(wdata);
        console.log(jsonObj);
        ...
        if(jsonObj.status === "200") {
          ...
          this.restProvider.registrAcessoApp("SAT", this.usuario.id, this.appBuild, versaoControle)
          .then((data: any) => {
            const wdata: any = JSON.stringify(data);
            const jsonObj: any = JSON.parse(wdata);
            console.log(jsonObj);
            ...
            buttons: [
              {
                ...
                role: 'Não',
                handler: () => {
                  this.ControleAtualizacaoUsuarioApp("SAT", this.usuario.id, this.appBuild, versaoControle, 'N', (async (wRetAtualApp: any) => {
  ...
  async ControleAtualizacaoUsuarioApp(appName: string, idUsuario: string, appBuild: string, buildCad: string, ctAtualizou: string, callback: any): Promise<any> {
    this.restProvider.controleAtualizacaoUsuarioApp("SAT", this.usuario.id, this.appBuild, buildCad, ctAtualizou)
    .then((data: any) => {
      const wdata: any = JSON.stringify(data);
      const jsonObj: any = JSON.parse(wdata);
      console.log(jsonObj);
  ...
  async RegistrAcessoApp(appName: string, idUsuario: string, appBuild: string, buildCad: string, callback: any): Promise<any> {
    this.restProvider.registrAcessoApp("SAT", this.usuario.id, this.appBuild, buildCad)
    .then((data: any) => {
      const wdata: any = JSON.stringify(data);
      const jsonObj: any = JSON.parse(wdata);
      console.log(jsonObj);
  ...
  async dadosUsuario(): Promise<boolean> {
    let wRet:any = false;
    const wdata: any = JSON.stringify(await this.restProvider.dadosUsuario(this.usuario.id, "SAT", this.usuario.token));
    const jsonObj: any = JSON.parse(wdata);
  ...
  async periodoAvaliacao(callback: any): Promise<any> {
    ...
    const PesquisaAtiva = await this.restProvider.periodoPesquisa(this.usuario.login, this.usuario.token, 'SATCM');
